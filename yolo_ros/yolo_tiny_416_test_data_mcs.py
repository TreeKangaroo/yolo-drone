...
class yolov4(object):
    ...
    def detect(self):
        ...
        for i in range(0,7620):
            ...
            try:
                ...
                yolo_result = False
                boxes, confs, clss = self.trt_yolo.detect(color_img, self.conf_th, last_coordinates)

                coordinates = self.publisher(color_img, depth_img, boxes, confs, clss)
                
                """
                if coordinates[0]!=-1 and last_coordinates[0]!=-1:
    ...
    def publisher(self, color_img, depth_img, boxes, confs, clss):
        ...
        if bbox[0] != -1:
            ...
            distance =0
            info_list=[int((x1+x2)/2), int((y1+y2)/2), x2-x1,conf]
            #image = Image()
        else:
            #image = Image()
            info_list=[-1,-1,-1,-1]
